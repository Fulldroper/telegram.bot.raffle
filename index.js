...
(async () => {
  ...
  const bot = new (await require("node-telegram-bot-api"))(process.env.TOKEN, {
    polling: true,
  });
  const axios = require("axios");
  bot.defaultLang = require("./lang.json")
  ...
  bot.lang = bot.addons["langSupport"]
  ...
  config.admins.forEach((chat_id) => {
    axios.post(
      `https://api.telegram.org/bot${process.env.TOKEN}/setMyCommands`,
      {
  ...
  if (!(await bot.db.get(`${bot.name}:lang`) || false)) {
    await bot.db.set(`${bot.name}:lang`, bot.defaultLang)
  }
  ...
  fastify.get(
    ...
    async function ({ params, query }, reply) {
      try {
        const { userID, orderID } = params;
        const user = await bot.getChat(userID);
        const { count } = query;
        // next
        const tikets = await bot.newTicket({
          count,
          user,
        });
        await bot.db.push(`${bot.name}:${userID}:tikets`, tikets);
        let str = `✅${count} купон(а) успешно добавлены к Вам в профиль:\n`;
        tikets.forEach((t) => (str += `\n${t}`));
        ...
        console.error(error);
        reply.send({ err: "wrong data input" });
      }
    }
  );

  fastify.listen(
    { port: process.env.PORT || 80, host: process.env.HOST || "0.0.0.0" },
    function (err, address) {
      if (err) {
        fastify.log.error(err);
        process.exit(1);
      }
      console.log(`[REST] started on ${address}`);
    }
  );
})();
